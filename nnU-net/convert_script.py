...
def make_out_dirs(dataset_id: int, task_name="ACDC"):
    ...
    out_test_labels_dir = out_dir / "labelsTs"  # Added test labels directory

    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(out_train_dir, exist_ok=True)
    os.makedirs(out_labels_dir, exist_ok=True)
    os.makedirs(out_test_dir, exist_ok=True)
    os.makedirs(out_test_labels_dir, exist_ok=True)  # Ensure test labels dir is created
...
def copy_files(src_data_folder: Path, train_dir: Path, labels_dir: Path, test_dir: Path, test_labels_dir: Path):
    """Copy files from the ACDC dataset to the nnUNet dataset folder, including test labels.
    Returns the number of training cases."""
    ...
    # Copy test files and corresponding labels.
    for patient_dir in patients_test:
        for file in patient_dir.iterdir():
            if file.suffix == ".gz" and "_gt" not in file.name and "_4d" not in file.name:
                shutil.copy(file, test_dir / f"{file.stem.split('.')[0]}_0000.nii.gz")
            elif file.suffix == ".gz" and "_gt" in file.name:
                shutil.copy(file, test_labels_dir / file.name.replace("_gt", ""))  # Save test labels
